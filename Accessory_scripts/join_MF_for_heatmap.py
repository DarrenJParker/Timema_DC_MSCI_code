...
for line in in_sex_file:
	...
	if line_N > 1:
		...
		gene_tiss_sp_MF_dict[(gene_id, tiss, sp)] = log2MF
...
tissue_order  = ['A', 'B', 'DG', 'FB', 'Fe', 'Ta',  'Gu',  'GoAG', 'GoTe']
...
my_tiss_order = []
for el in tissue_order :
	if el in seen_tiss:
		my_tiss_order.append(el)
print(my_tiss_order)
...
for gene in gene_ls_2:
	...
	MF_list = ""
	MF_samps = ""
	for t in my_tiss_order:
		for s in my_sp_order:
			rec = gene_tiss_sp_MF_dict.get((gene_id, t, s))
			if rec == None:
				rec = "NA"

			MF_list = MF_list + "," + rec
			MF_samps = MF_samps + "," + s + "_" + t
			
	if line_N == 1:
		out_file.write("gene_id,scaf,XA,mid_point" + MF_samps + "\n")
	
	out_file.write(gene_id + "," + gene_info[0]  + "," +  gene_info[1]  + "," +  gene_info[2]  + MF_list + "\n")
