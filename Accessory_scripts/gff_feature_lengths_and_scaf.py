...
try:
	opts, args = getopt.getopt(sys.argv[1:], 'i:o:f:p:g:h')
...
for opt, arg in opts:
	if opt in ('-h', '--help'):
		...
		print("Gets lengths of features in a gff file, summed by a group ID (e.g. exons lengths by gene) and scaffold for the gene")
		...
		print("-g\tgene experession file (csv). specify if you to add lens and scaf to the file - Default: off")
...
for line in in_gff:
	if not line.startswith("#"):
		...
		if feature == feature_want:
			...
			scaf        = line.split("\t")[0]
			feature_scaf_dict[grouping_feature] = scaf
...
out_file.write("Id,total_" + feature_want + "_length,N_" + feature_want + ",scaf\n")

for el in feature_len_dict:
	f_len = feature_len_dict.get(el)[0]
	n_feat = feature_len_dict.get(el)[1]
	scaf   = feature_scaf_dict.get(el)
	out_file.write(el + "," + str(f_len) + "," + str(n_feat) + "," +  scaf + "\n")

out_file.close()



if gene_exp_file_name != None:
	
	out_gene_expression_file = open(gene_exp_file_name.replace(".csv", "_wGLS.csv"), "w")	
	gene_exp_file = open(gene_exp_file_name)
	...
	for line in gene_exp_file:
		...
		line = line.strip()
		if line_N == 1:
			out_gene_expression_file.write(line + ",total_" + feature_want + "_length,N_" + feature_want + ",scaf\n")
		else:
			line = line.strip()
			gene_id = line.split(",")[0]
			# print(line)
			# print(gene_id)
			f_len = feature_len_dict.get(gene_id )[0]
			n_feat = feature_len_dict.get(gene_id )[1]
			scaf   = feature_scaf_dict.get(gene_id )
			out_gene_expression_file.write(line + "," + str(f_len) + "," + str(n_feat) + "," +  scaf + "\n")		
